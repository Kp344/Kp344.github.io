          ...
          "blocks": [
            {
              ...
              "caption": "Design 1 — paper cone with straw legs that fold on impact."
            },
            {
              "type": "image",
              "src": "",
              "alt": "Straw cage design",
              "caption": "Design 2 — straw cage that holds the egg away from every wall."
            },
            {
              "type": "image",
              "src": "",
...
function wireDrag(tree) {
  tree.querySelectorAll("[data-drag]").forEach((el) => {
    ...
    el.addEventListener("dragend", () => { el.classList.remove("dragging"); clearMarks(); dragInfo = null; });

    el.addEventListener("dragover", (ev) => {
      if (!dragInfo || dragInfo.kind !== el.dataset.drag) return;
      ev.preventDefault(); ev.stopPropagation();
      const r = el.getBoundingClientRect();
      clearMarks();
      el.classList.add(ev.clientY > r.top + r.height / 2 ? "drop-after" : "drop-before");
    });
    el.addEventListener("drop", (ev) => {
      if (!dragInfo || dragInfo.kind !== el.dataset.drag) return;
      ev.preventDefault(); ev.stopPropagation();
      const r = el.getBoundingClientRect();
      dropOnto(el.dataset.drag, +el.dataset.i, +el.dataset.j, +el.dataset.k, ev.clientY > r.top + r.height / 2);
    });
  });

  // dropping into a container: a part accepts blocks, a section accepts parts
  tree.querySelectorAll("[data-drop]").forEach((box) => {
    box.addEventListener("dragover", (ev) => {
      if (!dragInfo) return;
      const wants = box.dataset.drop === "sec" ? "sub" : "blk";
      if (dragInfo.kind !== wants) return;
      ev.preventDefault();
      clearMarks(); box.classList.add("drop-into");
    });
    box.addEventListener("drop", (ev) => {
      ...
      if (dragInfo.kind !== wants) return;
      ev.preventDefault();
      appendInto(wants, +box.dataset.i, +box.dataset.j);
    });
  });
}
